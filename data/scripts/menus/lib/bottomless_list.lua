...
function builder:build(props)
  ...
  --Enable multi_events for the menu so on_started, etc can be defined differently for each instance
  ...
  function menu:get_current_item()
    return menu.held_items[menu.cursor_index + 1].item
  end
  ...
  menu:register_event("on_draw", function(self, dst)
    ...
    elseif menu.items_surface.current_y > target_y then
  	...
  	cursor_sprite:draw(menu.menu_surface, cursor_sprite.x + padding + 16, cursor_sprite.y + padding + 21)
