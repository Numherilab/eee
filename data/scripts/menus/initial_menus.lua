...
for i,menu_script in ipairs(MENU_LIST) do
	...
	menu:register_event("on_finished", function(self)
		...
		if active_menu and next_menu and not sol.menu.is_started(next_menu) then --active_menu is nil if a game was started or initial_menus.stop() was called
...
function initial_menus.start(context, on_top)
	...
	if MENU_LIST.music then
    sol.timer.start(sol.main, 200, function()
      sol.audio.play_music(MENU_LIST.music)
    end)
  end
end
