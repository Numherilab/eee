...
function rain_manager:on_created()
  ...
  dark_surface:fill_color(color_darkness)
  dark_surface:set_opacity(100)
...
--Stock angled rain, taken out for more straight falling rain
--  drop.angle = 7 * math.pi / 5 + (deviation or 0)
  drop.angle = (3*math.pi/2)
...
function rain_manager:update_darkness()
  ...
  elseif current_rain_mode == "rain" then
    darkness = math.random(min_darkness, max_darkness/(math.random(2, 3)))
  end
  ...
  timers["darkness_timer"] = sol.timer.start(current_game, 45, function()
    ...
    dark_surface:set_opacity(current_darkness)
