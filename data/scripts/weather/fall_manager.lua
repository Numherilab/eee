...
The functions here defined are:
    ...
    game:set_fall_leaf_color({r,g,b})
...
local leaf_sprite = sol.sprite.create("weather/leaf_2")
leaf_sprite.num_frames = leaf_sprite:get_num_frames()
local leaf_framerate = 150
...
local leaf_min_zigzag_distance, leaf_max_zigzag_distance = 200, 500
local fall_leaf_delay, fall_storm_leaf_delay = 10, 5 -- In milliseconds.
local min_darkness, max_darkness = 1, 10 -- Opacity during fall_storm.
local current_darkness = 0 -- Opacity (transparent = 0, opaque = 255).
local color_darkness = {248, 96, 40} -- Used for full darkness.
local max_num_leaves_fall, max_num_leaves_fall_storm = 2, 10
...
function game_meta:set_fall_mode(fall_mode)
  ...
  leaf_sprite:set_color_modulation({255,255,255})
...
function game_meta:set_fall_leaf_color(rgb_value)
  leaf_sprite:set_color_modulation(rgb_value)
end
...
function fall_manager:on_created()
  ...
  fall_surface:set_blend_mode("blend")
  dark_surface:set_blend_mode("multiply")
  leaf_surface = sol.surface.create(16, 16)
...
function fall_manager:start_fall_mode(fall_mode)
  ...
  if timers["leaf_frame_timer"] == nil then
    timers["leaf_frame_timer"] = sol.timer.start(game, leaf_framerate, function()
      for _, leaf in pairs(leaf_list) do
        if leaf.exists then
          leaf.frame = (leaf.frame + 1) % leaf_sprite.num_frames
        end
      end
      return true
    end)
  ...
  if timers["leaf_opacity_timer"] == nil then
    timers["leaf_opacity_timer"] = sol.timer.start(game, 20, function()
      for _, leaf in pairs(leaf_list) do
        if leaf.exists then
