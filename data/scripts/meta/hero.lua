...
--alter damage for hard mode
--max life possible is 52
local function get_hard_mode_damage(damage)
  local MIN_MULT = 1.5 --minimum damage will be multiplied by
  local MAX_MULT = 3
  local UPPER_DAMAGE = 20 --damage above this amount will be multiplied by the minimum
  adjusted_damage = damage*math.max( (damage-2)*(MIN_MULT-MAX_MULT) / (UPPER_DAMAGE-2) + MAX_MULT, MIN_MULT )
  --print("old/new damage", damage, adjusted_damage)
...
function hero_meta:on_taking_damage(damage)
  ...
  --if in hard mode,  adjust damage taken accordingly
  if game:get_value"hard_mode" and game:get_value"hard_mode_enemy_damage" then
    damage = get_hard_mode_damage(damage)
  end

  --For no-hit challenge
  game.cursed_marble_no_hit_check = false
  ...
  local iframe_length = 1100
  if game:get_value"hard_mode" and game:get_value"hard_mode_iframes" then
    iframe_length = 700
  end
...
function hero_meta:on_state_changed(state)
  ...
  --Extend iframes for back to solid ground, since most of the time is taken up by the movement
  elseif state == "back to solid ground" then
