...
item:register_event("on_started", function(self)
  item:set_savegame_variable("possession_abyssal_flame")
...
--        sprite = "entities/flame_pillar",
        ...
        flame_pillars[i]:set_layer_independent_collisions(true)
        flame_pillars[i]:set_layer(map:get_max_layer())
  --      flame_pillars[i]:set_can_traverse_ground("wall", true)
      ...
      sol.timer.start(map, 500, function()
        ...
        for i = 9, 12 do
          ...
          flame_pillars[i]:set_layer_independent_collisions(true)
          flame_pillars[i]:set_layer(map:get_max_layer())
          -- flame_pillars[i]:get_sprite():set_blend_mode"add"
        end
      end)
      ...
      for enemy in map:get_entities_by_type"enemy" do
        if enemy:is_on_screen() then
          local x,y,z = enemy:get_position()
          local effect = map:create_custom_entity{width=16,height=16,direction=0,
            x=x, y=y, layer=layer, model="ephemeral_effect", sprite="enemies/misc/curse_blast"
          }
          effect:get_sprite():set_animation("charging", "burning")
          if not string.match(enemy:get_breed(), "misc")
          or string.match(enemy:get_breed(), "root")
          or string.match(enemy:get_breed(), "seamonster")
          or string.match(enemy:get_breed(), "skeleton_hand") then
            enemy:hurt(game:get_value("sword_damage") * (2+1))
          end
        end
      end
